...
import { Search, Plus, Phone, Mail, Building, User, CreditCard as Edit, Trash2, Globe, Filter, Trash, Upload, Timer, Clock, Calendar } from 'lucide-react';
...
const LeadsList: React.FC = () => {
  ...
  const calculateLeadTimer = (createdAt: string) => {
    const createdDate = new Date(createdAt);
    const now = new Date(currentTime); // Use state-based current time
    ...
    if (timeRemaining <= 0) {
      ...
      return {
        ...
        secondsRemaining: -overdueSeconds,
        isOverdue: true
      };
    } else {
      // Lead still has time
  ...
  const formatTimeRemaining = (createdAt: string) => {
    const timer = calculateLeadTimer(createdAt);
    
    if (timer.isOverdue) {
      const days = Math.abs(timer.daysRemaining);
      const hours = Math.abs(timer.hoursRemaining);
      const minutes = Math.abs(timer.minutesRemaining);
      const seconds = Math.abs(timer.secondsRemaining);
      
      if (days > 0) {
        return `${days}d ${hours}h ${minutes}m ${seconds}s overdue`;
      } else if (hours > 0) {
      ...
      const { daysRemaining, hoursRemaining, minutesRemaining, secondsRemaining } = timer;
      
      if (daysRemaining > 0) {
        return `${daysRemaining}d ${hoursRemaining}h ${minutesRemaining}m ${secondsRemaining}s left`;
  ...
  const handleScheduleUpdate = async (leadId: string, scheduledCall: string) => {
    const lead = leads.find(l => l.id === leadId);
    logActivity({
      actionType: 'edit',
      actionDetails: `Updated scheduled call for lead: ${lead?.name}`,
      targetType: 'lead',
      targetId: leadId,
      targetName: lead?.name,
      metadata: { 
        field: 'scheduled_call',
        new_value: scheduledCall,
        company: lead?.company
      }
    });
    ...
    try {
      const { error } = await supabase
        .from('leads')
        .update({ 
          scheduled_call: scheduledCall ? new Date(scheduledCall).toISOString() : null 
        })
        .eq('id', leadId);
    ...
    <div className="p-3 lg:p-6 space-y-4 lg:space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-green-400 mt-1">Manage your sales leads and prospects</p>
        </div>
        <div className="flex flex-wrap gap-2 lg:gap-3 items-center">
          <LeadNotifications onRefreshLeads={refetch} />
          ...
          <button 
            ...
            className="tech-button bg-purple-600 text-white px-2 lg:px-4 py-2 rounded-lg hover:bg-purple-700 transition-colors duration-200 flex items-center space-x-1 lg:space-x-2 text-xs lg:text-sm"
          >
          ...
          <table className="w-full">
            <thead className="bg-green-900 bg-opacity-30 border-b border-green-800">
              <tr>
                ...
                <th className="text-left py-2 px-1 lg:px-2 font-semibold text-green-300 text-xs hidden lg:table-cell">SC</th>
                <th className="text-left py-2 px-1 lg:px-2 font-semibold text-green-300 text-xs">Timer</th>
            ...
            <tbody>
              {filteredLeads.map((lead, index) => (
                <tr key={lead.id} className={`border-b border-green-800 hover:bg-green-900 hover:bg-opacity-20 transition-colors duration-150 ${
                  selectedLeads.has(lead.id) ? 'bg-green-900 bg-opacity-30' : 
                  (lead.status === 'prospect' && calculateLeadTimer(lead.createdAt).isOverdue) ? 'bg-red-900 bg-opacity-20' :
                    ...
                    {lead.scheduled_call ? (
                      <div className="space-y-1">
                        <div className="text-xs text-green-300 font-medium">
                          {new Date(lead.scheduled_call).toLocaleDateString('en-US', { 
                            month: 'short', 
                            day: 'numeric',
                            year: '2-digit'
                          })}
                        ...
                        <div className="text-xs text-green-400">
                          {new Date(lead.scheduled_call).toLocaleTimeString('en-US', { 
                            hour: '2-digit', 
                            minute: '2-digit',
                            hour12: false
                          })}
                        ...
                        {(() => {
                          ...
                          const isToday = new Date(lead.scheduled_call).toDateString() === new Date().toDateString();
                          
                          if (isOverdue) {
                            return <div className="text-xs text-red-400 font-medium">Overdue</div>;
                          } else if (isToday) {
                            return <div className="text-xs text-yellow-400 font-medium animate-pulse">Today</div>;
                          } else if (isPending) {
                            return <div className="text-xs text-green-500">Scheduled</div>;
                          }
                          return null;
                        })()}
                      </div>
                    ) : (
                      <span className="text-xs text-green-500">Not scheduled</span>
                    )}
                  </td>
                  <td className="py-2 lg:py-3 px-1 lg:px-2">
                    {lead.status === 'prospect' ? (
                      <div className="flex items-center space-x-1">
                        {(() => {
                          const timer = calculateLeadTimer(lead.createdAt);
                          const TimerIcon = timer.isOverdue ? Timer : Clock;
                          const timerColor = timer.isOverdue ? 'text-red-400' : 'text-green-400';
                          const bgColor = timer.isOverdue ? 'bg-red-900 bg-opacity-30' : 'bg-green-900 bg-opacity-30';
                          
                          return (
                            <div className={`flex items-center space-x-1 px-2 py-1 rounded text-xs ${bgColor} ${timerColor} ${timer.isOverdue ? 'animate-pulse' : ''}`}>
                              <TimerIcon className="w-3 h-3" />
                              <span className="font-medium">
                                {formatTimeRemaining(lead.createdAt)}
                              </span>
                            </div>
                          );
                        })()}
                  ...
                  <td className="py-2 lg:py-3 px-1 lg:px-2">
                    <div className="flex space-x-1">
                      ...
                      <button
                        ...
                        className="p-1 lg:p-1.5 text-green-400 hover:text-blue-400 transition-colors rounded hover:bg-blue-900 hover:bg-opacity-30"
                        title="Schedule Call"
                      >
